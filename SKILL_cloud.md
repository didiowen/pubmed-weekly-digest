...
> ⚠️ **SKILL_cloud.md — 通用版本**（cloud 環境與本地端均可使用）
> 本地端執行如需更高的 newsletter DOI 解析成功率，可改用 `SKILL_local.md`：增加 Subagent B Step B3.0「追蹤 URL redirect → DOI 解析」。cloud sandbox 的 TLS proxy 會阻擋 `t.n.nejm.org` 的跨網域 redirect，故該步驟保留於本地端版本。
...
> 解析失敗的單篇：跳過並回報。
>
> **Step B3 — 對每篇 newsletter 文章做 PubMed/CrossRef 標題回查**（取得 DOI 與 abstract）：
>
> > **重要**：所有 article type（Editorial、Perspective、Correspondence 等）**一律執行**，不得跳過。
>
> 追蹤 URL 伺服器端 fetch 已知回傳 HTTP 500，**勿嘗試 WebFetch**。
>
...
### Step C2.5 — 缺摘要的 OA 全文回退（PubMed 與 newsletter 皆適用）
...
3. **非 OA／付費牆／取得失敗** → 不杜撰，維持 Step 4 表格的 `（無摘要）` 標註。（本地端如需透過機構帳號取付費全文，請改用 `SKILL_local.md`。）
4. 盡力而為：任何 API 或 WebFetch 失敗皆**不中止** routine。

> **環境註記**：cloud sandbox 對外 fetch 或 TLS proxy 可能受限；OA 判斷（API 查詢）通常仍可行，實際全文 WebFetch 視環境而定，失敗則回到缺摘要標註。
...
- `peripheral blood` 譯為「周邊血」（台灣用語），**不用「外周血」**（中國用語）
...
- 文中若提及病原屬名/種名以 markdown 斜體標示（如 *Candida*、*A. baumannii*），疾病名與病毒縮寫不用斜體；Salmonella serovar（大寫 Typhi/Enteritidis 等）只斜體屬名（*S.* Typhi），接小寫才整串斜體（*Salmonella typhosa*）
...
> 落入上表「留空／無摘要」前，須先經 **Step C2.5** 以 DOI 查 OA 全文；OA 命中則由全文補 TL;DR，僅在非 OA／付費牆時才標「無摘要」。
...
- Newsletter 文章：DOI 行一律改為 `[**NEJM Newsletter**]({tracking_url})`。若 tracking_url 無法提取，則保留 `**DOI**: 無 PubMed/CrossRef 紀錄`。
